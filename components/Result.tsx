...
export const Result: React.FC<Props> = ({ answers }) => {
    ...
    const handleDirectWhatsappSubmit = async (e: React.FormEvent) => {
        ...
        if (directWhatsappName && directWhatsappEmail && directWhatsappNumber) {
            await saveLead(directWhatsappName, directWhatsappEmail, directWhatsappNumber, 'WhatsApp Direct');
            // Use window.open with _blank because wa.me links often cannot be loaded inside iframes (X-Frame-Options)
            // This ensures the user is successfully taken to WhatsApp
            window.open("https://wa.me/message/VFMAYP65ATMVL1", "_blank");
        }
    };
