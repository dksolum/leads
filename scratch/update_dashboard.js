...
if (!content.includes(targetFunctionAnchor)) {
    console.error('Erro: Âncora da função de ordenação não encontrada!');
    process.exit(1);
}

content = content.replace(targetFunctionAnchor, targetFunctionAnchor + functionInsertion);
...
// Normalizar quebras de linha para evitar incompatibilidades de CRLF vs LF
const normalize = str => str.replace(/\r\n/g, '\n').trim();

const normalizedContent = normalize(content);
const normalizedTarget = normalize(targetPricingBlock);

if (!normalizedContent.includes(normalizedTarget)) {
    console.error('Erro: Bloco de precificação original não encontrado no arquivo!');
    process.exit(1);
}

// Fazer o replace no conteúdo com quebras de linha normais
// Para garantir que o replace funcione independente de \r\n vs \n,
// vamos fazer o replace usando as strings normalizadas e salvar
const newContent = content.replace(/\r\n/g, '\n').replace(targetPricingBlock.replace(/\r\n/g, '\n'), newPricingBlock.replace(/\r\n/g, '\n'));
fs.writeFileSync(filePath, newContent, 'utf8');
console.log('AdminDashboard.tsx atualizado com sucesso via script!');
